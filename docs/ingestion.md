...
현재 제공되는 Adapter는 `sindoh-plain`, `kyocera`/`kyocera-ocr`, `samsung-rtf`입니다.
메일 저장 직후 자동 실행되며 시리얼을 공백·기호·대소문자와 무관하게 등록 장비와 대조합니다.
흑백과 컬러만 명시된 경우 총 카운터는 두 값의 합으로 보완하되 신뢰도를 낮춥니다. 세 카운터가
모두 있고 신뢰도가 0.9 이상인 평문/RTF 결과는 자동 확정됩니다. OCR 결과, 필드 누락, 미등록
시리얼, 직전 확정값보다 작은 값은 검수 대상으로 남고 처리 이벤트에 사유를 기록합니다.
