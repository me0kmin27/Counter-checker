...
Adapter는 운영 데이터로 임의의 값을 추측하는 머신러닝 모델이 아니라 버전이 고정된 템플릿
파서입니다. 특히 교세라 본문 예시처럼 컬러·흑백 구분값이 없는 메일은 총 카운터만 저장하고
없는 값을 만들어내지 않습니다. 새 실메일 양식과 운영자 검수 결과는 개인정보를 제거한 회귀
fixture로 추가하고 Adapter 버전을 올리는 방식으로 누적 학습 효과와 결과 재현성을 함께
확보합니다.
