...
- `자동`: SMTP 포트 465에서는 SSL/TLS, 포트 587에서는 STARTTLS를 사용하고, 그 밖의 포트에서는
  암호화를 사용하지 않습니다.
...
- `사용 안 함`: SMTP 연결을 암호화하지 않습니다. 자격 증명이 평문으로 노출될 수 있으므로 내부의
  신뢰할 수 있는 서버에서만 사용하고, 인터넷 SMTP 서버에는 선택하지 마십시오.
