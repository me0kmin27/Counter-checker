...
- `사용 안 함`: TLS 없이 연결합니다. 서버가 암호화 연결을 지원하지 않을 때만 사용합니다.
...
- **POP 서버 응답 `line too long`**: 일부 POP 서버는 MIME 본문이나 첨부 데이터를 표준 라이브러리의
  기본 2 KiB보다 긴 한 줄로 전송합니다. 수신기는 전역 `poplib` 설정을 변경하지 않고 메시지 저장
  한도인 25 MiB까지 한 줄을 허용합니다. 이 한도를 넘는 비정상 응답은 메모리 사용을 제한하기 위해
  거부합니다.
