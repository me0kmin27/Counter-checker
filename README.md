...
수신 메일은 신도리코 평문, 교세라 평문·스크린샷, 삼성 RTF 카운터 페이지 형식을
자동 판별합니다. 등록 장비의 시리얼과 일치하면 흑백·컬러·총 카운터를 자동 확정하고,
누락·미등록 시리얼·카운터 감소는 검수 대상으로 보존합니다. 교세라 이미지 OCR은 실행
환경에 Tesseract(`kor`, `eng` 언어 데이터)가 있을 때 사용하며 평문을 항상 우선합니다.
